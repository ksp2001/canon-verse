...
function ArtistInput() {
    ...
    const prompt = `Give me a song as if it was written by ${artist}, about my friend ${name} who I describe as ${qualities}. Do not make any allusions to romance in this, this is my friend. Do not assume gender. Make sure it's as ${artist}-y as it gets, and give me a creative title, the first stanza, the chorus, and the bridge. Give it to me as a JSON response: {"title": "my song title", "lyrics": "my song lyrics1"}. Only provide a RFC8259 compliant JSON response following this format without deviation. If you do not follow this format, I will not be able to parse your response. If there are any newlines in the lyrics string, please replace them with the string "\\\\n".`

    const handleSubmit = async (e) => {
        e.preventDefault();
        if (artist === "" || name === "" || qualities === "") return alert("Please enter an artist name.")
        ...
        fetch(process.env.REACT_APP_API_ENDPOINT, options)
            ...
            .then(data => setApiResponse({response: data.choices[0].text, qualities: qualities.split(','), artist: artist, name: name}))
    ...
    return (
        <>
            {!loading && apiResponse === '' &&
                <div className="bg-gradient-to-b from-neutral-900 to-neutral-950 w-full h-full flex justify-center items-center">
                    <div className="bg-black font-sans text-3xl flex rounded-lg py-24 md:px-24 md:mt-8 w-fit items-center">
                        <div className='md:px-16 px-0'>
                            <div className="p-0 m-0 text-left mb-0 pb-0 md:mx-0 mx-6 w-fit">
                                <h1 className="text-5xl font-bold text-white mb-12 text-left">Write me a song</h1>
                                <h6 className="text-sm font-bold text-white mb-2 text-left">Artist's Name</h6>
                                <input className="placeholder:text-neutral-400 placeholder:text-base text-neutral-400 text-base bg-neutral-700 border border-neutral-400 rounded-sm focus:outline-none px-2 py-1 mb-4 w-full" type="text" placeholder="Artist's name" value={artist} onChange={(e) => setArtist(e.target.value)} /><br />
                                <h6 className="text-sm font-bold text-white mb-2 text-left">Friend's Name</h6>
                                <input className="placeholder:text-neutral-400 placeholder:text-base text-neutral-400 text-base bg-neutral-700 border border-neutral-400 rounded-sm focus:outline-none px-2 py-1 mb-4 w-full" type="text" placeholder="Friend's name" value={name} onChange={(e) => setName(e.target.value)} /><br />
                                <h6 className="text-sm font-bold text-white mb-2 text-left">Friend's Qualities</h6>
                                <input className="placeholder:text-neutral-400 placeholder:text-base text-neutral-400 text-base bg-neutral-700 border border-neutral-400 rounded-sm focus:outline-none px-2 py-1 mb-4 w-full" type="text" placeholder="Friend's qualities" value={qualities} onChange={(e) => setQualities(e.target.value)} /><br />
                                <button className="w-full rounded-full hover:scale-105 bg-green-600 py-3 px-3 my-2 self-center text-sm font-bold text-black" onClick={handleSubmit}>Generate</button>
                            </div>
                        </div>
                    </div>
                </div>
